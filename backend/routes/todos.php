...
$router->addRoute('GET', '/^\/api\/todos$/', function() {
    ...
    try {
        ...
        // Get todos
        $query = "SELECT t.id, t.title, t.description, t.is_completed, t.total_steps, 
                         t.completed_steps, t.completed_at, t.created_at, t.updated_at
                  FROM todos t
                  WHERE $whereClause
        ...
        foreach ($todos as &$todo) {
            ...
            $todo['progress_text'] = $todo['completed_steps'] . '/' . $todo['total_steps'];
            $todo['progress_percentage'] = $todo['total_steps'] > 0 ? 
                round(($todo['completed_steps'] / $todo['total_steps']) * 100) : 0;
        }
...
$router->addRoute('POST', '/^\/api\/todos$/', function() {
    ...
    try {
        $query = "INSERT INTO todos (user_id, title, description, total_steps, completed_steps) 
                  VALUES (:user_id, :title, :description, :total_steps, :completed_steps)";
        
        $stmt = $db->prepare($query);
        
        $totalSteps = isset($input['total_steps']) && $input['total_steps'] > 0 ? (int)$input['total_steps'] : 1;
        $completedSteps = isset($input['completed_steps']) && $input['completed_steps'] >= 0 ? 
            min((int)$input['completed_steps'], $totalSteps) : 0;
        
        $stmt->bindParam(':user_id', $userId);
        $stmt->bindParam(':title', $input['title']);
        $stmt->bindParam(':description', $input['description'] ?? '');
        $stmt->bindParam(':total_steps', $totalSteps);
        $stmt->bindParam(':completed_steps', $completedSteps);
        if ($stmt->execute()) {
            $todoId = $db->lastInsertId();
            
            // Get the created todo
            $getQuery = "SELECT * FROM todos WHERE id = :todo_id";
            ...
            $newTodo['progress_text'] = $newTodo['completed_steps'] . '/' . $newTodo['total_steps'];
            $newTodo['progress_percentage'] = $newTodo['total_steps'] > 0 ? 
                round(($newTodo['completed_steps'] / $newTodo['total_steps']) * 100) : 0;
...
$router->addRoute('PUT', '/^\/api\/todos\/(\d+)$/', function($todoId) {
    ...
    try {
        ...
        // Update the todo
        $query = "UPDATE todos 
                  SET title = :title, description = :description, total_steps = :total_steps, 
        ...
        if ($stmt->execute()) {
            ...
            $updatedTodo['progress_text'] = $updatedTodo['completed_steps'] . '/' . $updatedTodo['total_steps'];
            $updatedTodo['progress_percentage'] = $updatedTodo['total_steps'] > 0 ? 
                round(($updatedTodo['completed_steps'] / $updatedTodo['total_steps']) * 100) : 0;
...
$router->addRoute('PUT', '/^\/api\/todos\/(\d+)\/complete$/', function($todoId) {
    ...
    try {
        // Get current status and update completion based on steps
        $getQuery = "SELECT total_steps, completed_steps, is_completed FROM todos WHERE id = :todo_id AND user_id = :user_id";
        $getStmt = $db->prepare($getQuery);
        $getStmt->bindParam(':todo_id', $todoId);
        ...
        // If marking as complete, set completed_steps = total_steps
        $completedSteps = $newStatus ? $current['total_steps'] : 0;
        ...
        if ($stmt->execute()) {
            ...
            ApiResponse::success([
                ...
                'progress_text' => $completedSteps . '/' . $current['total_steps']
            ], $message);
        }
...
// Update todo progress (individual steps)
$router->addRoute('PUT', '/^\/api\/todos\/(\d+)\/progress$/', function($todoId) {
    Auth::requireAuth();
    ...
    if (!isset($input['completed_steps']) || $input['completed_steps'] < 0) {
        ApiResponse::error('Invalid completed_steps value');
    }
    
    try {
        // Get current todo
        $getQuery = "SELECT total_steps, completed_steps FROM todos WHERE id = :todo_id AND user_id = :user_id";
        $getStmt = $db->prepare($getQuery);
        $getStmt->bindParam(':todo_id', $todoId);
        $getStmt->bindParam(':user_id', $userId);
        $getStmt->execute();
        ...
        $completedSteps = min((int)$input['completed_steps'], $current['total_steps']);
        $isCompleted = $completedSteps >= $current['total_steps'];
        $completedAt = $isCompleted ? date('Y-m-d H:i:s') : null;
        
        // Update progress
        $query = "UPDATE todos SET completed_steps = :completed_steps, is_completed = :is_completed, 
        ...
        if ($stmt->execute()) {
            $message = $isCompleted ? 'Congratulations! All steps completed! 🎉✨' : 'Progress updated! 📈';
            ApiResponse::success([
